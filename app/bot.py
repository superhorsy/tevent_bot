...
from users import PickleDBUserAccessor
...
def remind():
    def _notify(chat_id, user):
        ...
        _send("3 суток прошли и ты снова можешь забрать свой бонус 1 час, пакет 4 часа, энергетик, Ягуар XF. Просто "
              "нажимай на 🎲 внизу экрана", chat_id)
...
def main():
    ...
    def show_help(message: telebot.types.Message):
        _send("📍 Участвовать в игре можно один раз в 72 часа.\n"
              ...
              "⛔️ Скриншоты и переотправленные сообщения в другие диалоги засчитаны не будут!", message.chat.id)

    @bot.message_handler(commands=['start'])
    def start(message: telebot.types.Message):
        if users.get(str(message.chat.id)):
            _send(f"Привет! Жми на 🎲, чтобы проверить, нет ли для тебя приза", message.chat.id)
            return
        _send(f"Привет {message.from_user.first_name}! \n{NOT_LOGGED_IN}", message.chat.id)

    @bot.message_handler(func=lambda message: not users.get(str(message.chat.id)) and message.text != 'Справка')
    def login(message: telebot.types.Message):
        input_phone = message.text
        print(f"Введено: {input_phone}")
        match = re.match('\+?[7,8]?([\s-]*\d{3}[\s-]*\d{3}[\s-]*\d{2}[\s-]*\d{2})', input_phone)
        if not match:
        ...
        search = f"\+?[7,8]([\s-]*{''.join(phone[0:3])}[\s-]*{''.join(phone[3:6])}[\s-]*{''.join(phone[6:8])}[\s-]*{''.join(phone[8:10])})"
        ...
        _send(f"Мы тебя узнали, {values[0]}! Теперь подыщем тебе промокод...", message.chat.id)
        get_promo(message)

    def get_promo(message: telebot.types.Message):
        user: string = users.get(str(message.chat.id))
        promo: Promo = Promo.find(user.phone)

        if promo and promo.is_valid():
            _send(f"Текущий промокод: \n"
                  f"🎫 Награда - {promo.award} \n"
                  f"🏷 Выдан {promo.date.strftime(DATETIME_FORMAT)} (МСК) \n"
                  f"🔐 Код {promo.code} \n", message.chat.id)
            _send(f"Новый промокод будет доступен {promo.next_date().strftime(DATETIME_FORMAT)}.", message.chat.id)
            return

        promo: Promo = Promo.new(user.phone)
        _send(f"Поздравляем! Ты выиграл {promo.award} 🎉 \n"
              f"🏷 Выдан {promo.date.strftime(DATETIME_FORMAT)} (МСК) \n"
              f"🔐 Код {promo.code} \n", message.chat.id)
        return
...
class Promo:
    def __init__(self, phone: string, code: string = None, award: string = None,
    ...
    @staticmethod
    def find_promo(phone: string):
        """row: 0 - phone, 1 - date, 2 - code, 3 - award """
        promo_rows: list = promo_wks.findall(in_column=1, query=phone)
        if not promo_rows:
            return None

        promo_values: list = list(map(lambda x: promo_wks.row_values(x.row), promo_rows))
        promo_values.sort(key=lambda x: datetime.strptime(x[1], DATETIME_FORMAT), reverse=True)

        date: datetime = datetime.strptime(promo_values[0][1], DATETIME_FORMAT)
        promo = Promo(code=promo_values[0][2], award=promo_values[0][3], date=date, phone=promo_values[0][0])

        return promo

    @staticmethod
    def find(phone: string):
    ...
    @staticmethod
    def new(phone: string):
        return Promo(phone=phone).save()

    @staticmethod
    def __generate_code() -> string:
        return ''.join(random.choices(string.ascii_uppercase + string.digits, k=10))

    @staticmethod
    def __generate_award() -> string:
        award_40_perc = '1 час'
    ...
    def save(self):
        promo_wks.append_row([self.phone, self.date.strftime(DATETIME_FORMAT), self.code, self.award])
        return self
